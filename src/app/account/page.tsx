...
export default function AccountPage() {
  ...
  const [suddenNotificationsEnabled, setSuddenNotificationsEnabled] = useState(false);
  const [suddenTestSent, setSuddenTestSent] = useState(false);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const storedSudden = localStorage.getItem("bac_sudden_notifications");
      setSuddenNotificationsEnabled(storedSudden === "enabled");
    }
  }, []);
  ...
  // Sudden Question notification toggle
  const handleToggleSuddenNotifications = async () => {
    const nextState = !suddenNotificationsEnabled;
    setSuddenNotificationsEnabled(nextState);
    if (typeof window !== "undefined") {
      localStorage.setItem("bac_sudden_notifications", nextState ? "enabled" : "disabled");
    }
  ...
  // Sample questions pool for Sudden Brain Ping
  const sampleSuddenQuestions = [
    { q: "ما هو تاريخ تأسيس حركة عدم الانحياز (مؤتمر بلغراد)؟", ans: "1 - 6 سبتمبر 1961" },
    { q: "من هو صاحب مبدأ ملء الفراغ وأول رئيس أمريكي زار الجزائر؟", ans: "دوايت أيزنهاور" },
    { q: "ما هو تاريخ هجومات الشمال القسنطيني والهدف منها؟", ans: "20 أوت 1955 بقيادة البطل زيغود يوسف" },
    { q: "ما هو تاريخ انعقاد مؤتمر الصومام وإعادة هيكلة الثورة؟", ans: "20 أوت 1956 بإفري أوزلاقن (بجاية)" },
    { q: "ما هو تاريخ إعلان مبدأ ترومان وسياسة الاحتواء والتطويق؟", ans: "12 مارس 1947" },
    { q: "ما هو تاريخ تأميم المحروقات واسترجاع السيادة النفطية للجزائر؟", ans: "24 فيفري 1971" },
    { q: "ما هو تاريخ تأسيس حلف الشمال الأطلسي (الناتو - NATO)؟", ans: "4 أفريل 1949" },
    { q: "من هو زعيم حركة التضامن ومفجر البيريسترويكا والغلاسنوست؟", ans: "ميخائيل غورباتشوف" },
  ];

  const handleSendSuddenQuestion = () => {
    if (typeof window === "undefined") return;
    const randomItem = sampleSuddenQuestions[Math.floor(Math.random() * sampleSuddenQuestions.length)];
    if ("Notification" in window && Notification.permission === "granted") {
      const title = isAr ? "⚡ سؤال فجائي لتنشيط الذاكرة (BAC 2027)" : "⚡ Question Flash BAC";
      const body = `${randomItem.q}\nاضغط الآن للتحقق من الإجابة في بنك الحفظ!`;
      try {
        ...
        setSuddenTestSent(true);
        setTimeout(() => setSuddenTestSent(false), 3500);
      } catch (e) {
        console.error("Error sending sudden notification:", e);
      }
    } else {
    ...
    <AppShell activeNav="account">
      <Container size="xl" className="py-6 sm:py-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          {/* ================================================================= */}
          {/* COLUMN 1: PRIMARY PROFILE, METRICS & CURRICULUM (7-8 COLS)        */}
          {/* ================================================================= */}
          <div className="lg:col-span-7 xl:col-span-8 space-y-6">
            ...
            {/* 4. STREAM-AWARE SUBJECT MASTERY & DIAGNOSTICS BREAKDOWN */}
            ...
            {/* 5. ACADEMIC PROFILE & CHARACTER DETAILS */}
            <Card className="p-5 space-y-4 shadow-card">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-bold text-theme-text flex items-center gap-2">
          ...
          {/* ================================================================= */}
          {/* COLUMN 2: SUBSCRIPTION, NOTIFICATIONS, TELEMETRY & LOGOUT (4-5)   */}
          {/* ================================================================= */}
          <div className="lg:col-span-5 xl:col-span-4 space-y-6 lg:sticky lg:top-20">
            ...
            <Card className="p-5 space-y-4 shadow-card">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-bold text-theme-text flex items-center gap-2">
                  ...
                  <span>{isAr ? "مركز الإشعارات والتنشيط الذهني" : "Notifications & Brain Ping"}</span>
                </h2>
              ...
              {/* Module A: Daily Study Reminders */}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] text-theme-muted leading-relaxed">
                      {isAr
                        ? "تنبيه خفيف يُرسل في موعد دراستك المفضل لتذكيرك بإنجاز مهمتك اليومية."
                ...
                <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-theme/60">
                  ...
                  >
                    ...
                    <span>
                      {notificationPermission === "granted"
                        ...
                        : (isAr ? "تفعيل إشعارات التذكير" : "Activer les rappels")}
                    </span>
                  </Button>
                  ...
                  {notificationPermission === "granted" && (
                    ...
                    >
                      ...
                      <span>{testNotificationSent ? (isAr ? "تم الإرسال ✓" : "Envoyé") : (isAr ? "تجربة الإشعار" : "Tester")}</span>
                    </Button>
                  )}
                </div>
              </div>

              {/* Module B: Sudden Brain Ping (السؤال الفجائي التنشيطي) */}
              <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/20 space-y-3">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-bold text-theme-text text-xs">
                        {isAr ? "السؤال الفجائي التنشيطي (حفظ وتثبيت)" : "Question Flash Mémoire"}
                      </span>
                    </div>
                    <p className="text-[11px] text-theme-muted leading-relaxed">
                      {isAr
                        ? "إشعار فجائي يرسل أسئلة سريعة في التواريخ والشخصيات لإبقاء عقلك نشطاً طوال اليوم."
                        : "Défis surprises de mémorisation envoyés pour stimuler votre mémoire vive."}
                    </p>
                  </div>

                  <span
                    className={`text-[11px] font-bold px-2 py-0.5 rounded-full shrink-0 ${
                      suddenNotificationsEnabled && notificationPermission === "granted"
                ...
                <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-amber-500/20">
                  <Button
                    size="sm"
                    variant={suddenNotificationsEnabled ? "outline" : "primary"}
                    onClick={handleToggleSuddenNotifications}
                    className="text-xs font-bold rounded-xl py-2 px-3 bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-500/30 hover:bg-amber-500/20"
                  >
                    <Zap className="w-3.5 h-3.5 me-1" />
                    <span>
                      {suddenNotificationsEnabled
                        ? (isAr ? "إيقاف السؤال الفجائي" : "Désactiver")
                        : (isAr ? "تفعيل السؤال الفجائي" : "Activer Question Flash")}
                    </span>
                  </Button>
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={handleSendSuddenQuestion}
                    className="text-xs rounded-xl py-2 px-3 border-amber-500/30 text-amber-700 dark:text-amber-300 hover:bg-amber-500/10"
                    title={isAr ? "إرسال سؤال فجائي تجريبي فوراً" : "Tester un défi flash"}
                  >
                    <Sparkles className="w-3.5 h-3.5 text-amber-500 me-1" />
                    <span>{suddenTestSent ? (isAr ? "تم الإرسال ⚡" : "Envoyé !") : (isAr ? "جرّب سؤالاً فجائياً الآن ⚡" : "Tester flash")}</span>
                  </Button>

                  <Link href="/memorize" className="w-full pt-1">
                    <Button
                      size="sm"
                      variant="ghost"
                      className="w-full text-xs text-[var(--color-primary)] hover:bg-[var(--color-primary-soft)] rounded-xl py-2 flex items-center justify-center gap-1.5"
                    >
                      <Brain className="w-3.5 h-3.5" />
                      <span>{isAr ? "فتح بنك ألعاب الحفظ والمسابقات 🧠" : "Ouvrir le jeu de mémorisation"}</span>
            ...
            {/* 8. PILOT TELEMETRY & SAFE DATA EXPORT */}
