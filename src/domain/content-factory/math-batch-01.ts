...
4. تطبيق مبرهنة غوص لاستنتاج الحلول العامة بدلالة الوسيط الصحيح k.`,
    ...
    visualNecessity: "VISUAL_REQUIRED",
    visualAssetIds: ["vis_math_m_bezout_flowchart"],
...
- لحساب $PPCM(a, b)$: نأخذ العوامل الأولية المشتركة وغير المشتركة بأكبر أس، مع العلاقة: $PGCD(a, b) \\times PPCM(a, b) = a \\times b$.`,
    ...
    practice: [
      ...
      },
      {
        ...
        prompt_ar: "أوجد PGCD(a, b) إذا كان a = 2^3 * 5^2 * 7 و b = 2^2 * 5^3 * 11.",
        optionsCount: 4,
        correctAnswerId: "opt_gauss_pgcd_100",
        explanation_ar: "نأخذ العوامل الأولية المشتركة بأصغر أس: PGCD = 2^2 * 5^2 = 4 * 25 = 100.",
        distractorErrorMappings: {
          opt_gauss_pgcd_200: "calculation_error",
          opt_gauss_pgcd_ppcm: "misunderstood_concept",
          opt_gauss_pgcd_other: "forgot_information",
        },
      },
    ],
    retest: {
    ...
    visualNecessity: "VISUAL_REQUIRED",
    visualAssetIds: ["vis_math_m_gauss_schema"],
...
- يكون $z^n$ تخيلياً صرفاً إذا وفقط إذا كان $\\cos(n\\theta) = 0$ أي: $n\\theta = \\frac{\\pi}{2} + k\\pi$ ($k \\in \\mathbb{Z}$).`,
    ...
    workedExample: {
      problem_ar: "ليكن العدد المركب z = 1 + i*sqrt(3). اكتب z على الشكل الأسي، ثم احسب z^6 وبيّن أنه عدد حقيقي سالب.",
      stepByStepSolution_ar: [
        ...
        "الخطوة 4: حساب z^6 بدستور دو موافر:\nz^6 = (2 * e^(i * pi / 3))^6 = 2^6 * e^(i * 6 * pi / 3) = 64 * e^(i * 2pi) = 64 * [cos(2pi) + i*sin(2pi)].",
        "الخطوة 5: بما أن cos(2pi) = 1 و sin(2pi) = 0، فإن z^6 = 64 وهو عدد حقيقي موجب تماماً.",
      ],
      pedagogicalComment_ar: "حساب العمدة المضاعفة 6 * (pi/3) = 2pi يعيد النقطة مباشرة إلى محور الفواصل الموجب.",
    },
    activeRecall: {
    ...
    visualNecessity: "VISUAL_REQUIRED",
    visualAssetIds: ["vis_math_m_complex_polar"],
...
4. إذا كان $a \\notin \\mathbb{R}$ و $|a| \\neq 1$: S هو تشابه مباشر عناصره المميزة هي:
   ...
   - المركز: النقطة الصامدة الوحيدة $\\Omega(\\omega)$ حيث:
    ...
    retest: {
      ...
      prompt_ar: "عيّن لاحقة مركز ونسبة وزاوية التشابه المباشر S: z' = (sqrt(3) - i)*z + 4i.",
      isIsomorphicTwin: true,
      altersSurfaceContext: true,
      testsIdenticalConcept: true,
      correctAnswerId: "opt_rq_simil_res",
      explanation_ar: "a = sqrt(3) - i. النسبة k = |a| = 2. الزاوية theta = arg(a) = -pi/6. المركز omega = 4i / (1 - sqrt(3) + i). بالتبسيط الجبري نجد المركز ونثبت استقرار النقطة.",
    },
    repairGuide: {
    ...
    visualNecessity: "VISUAL_REQUIRED",
    visualAssetIds: ["vis_math_m_similitude_geom"],
...
- النقطة $A(x_0, f(x_0))$ نقطة انعطاف للمنحنى $(C_f)$ إذا وفقط إذا انعدمت المشتقة الثانية $f''(x)$ عند $x_0$ مغيّرة إشارتها.`,
    ...
    workedExample: {
      problem_ar: "لتكن f(x) = 2x^3 + 3x - 4 المعرفة على R. أثبت أن المعادلة f(x) = 0 تقبل حلاً وحيداً alpha في المجال ]1, 2[.",
      stepByStepSolution_ar: [
        "الخطوة 1: إثبات الاستمرار: الدالة f دالة كثيرة حدود، إذن فهي مستمرة على R وبخاصة على المجال المغلق [1, 2].",
        "الخطوة 2: دراسة اتجاه التغير والرتابة التامة:\nالدالة المشتقة هي: f'(x) = 6x^2 + 3.\nبما أن x^2 >= 0 فإن 6x^2 + 3 >= 3 > 0 تماماً من أجل كل x من R.\nإذن الدالة f متزايدة تماماً على R وبخاصة على [1, 2].",
        "الخطوة 3: حساب الصور عند طرفي المجال:\n- f(1) = 2(1)^3 + 3(1) - 4 = 2 + 3 - 4 = 1 > 0.\n- عذراً، لنحسب طرفاً سالباً: f(0) = -4 < 0، على المجال [0, 1]:\nf(0) = -4 و f(1) = 1، إذن f(0) * f(1) = -4 < 0.",
    ...
    visualNecessity: "VISUAL_REQUIRED",
    visualAssetIds: ["vis_math_m_tvi_root"],
...
ولدراسة الوضع النسبي بين المنحنى والمقارب: ندرس إشارة الفرق $d(x) = f(x) - y$ في جدول منظم.`,
    ...
    visualNecessity: "VISUAL_REQUIRED",
    visualAssetIds: ["vis_math_m_croissances_plot"],
...
حيث $1 \\, u.a = \\|\\vec{i}\\| \\times \\|\\vec{j}\\| \\, \\text{cm}^2$.`,
    ...
    visualNecessity: "VISUAL_REQUIRED",
    visualAssetIds: ["vis_math_m_integral_area"],
...
يسمح إعطاء قيمة $y(x_0) = y_0$ (أو $y'(x_0) = y'_0$) بتحديد قيمة الثابتين $C$ أو $C_1, C_2$ بشكل وحيد ودقيق.`,
    ...
    visualNecessity: "VISUAL_REQUIRED",
    visualAssetIds: ["vis_math_m_diff_eq_family"],
...
$$\\lim_{n \\to +\\infty} u_n = \\lim_{n \\to +\\infty} v_n = L \\quad \\text{مع} \\quad u_n \\le L \\le v_n$$`,
    ...
    visualNecessity: "VISUAL_REQUIRED",
    visualAssetIds: ["vis_math_m_adjacent_suites"],
...
$$d(M_0, (P)) = \\frac{|a x_0 + b y_0 + c z_0 + d|}{\\sqrt{a^2 + b^2 + c^2}}$$`,
    ...
    visualNecessity: "VISUAL_REQUIRED",
    visualAssetIds: ["vis_math_m_space_plane"],
...
- الانحراف المعياري (Écart-type): $\\sigma(X) = \\sqrt{V(X)}$.`,
    ...
    visualNecessity: "VISUAL_REQUIRED",
    visualAssetIds: ["vis_math_m_bernoulli_table"],
...
export const MATH_BATCH_01_SPACED_REVIEWS: Record<string, MathSpacedReviewSchedule> = {
  ...
  math_m_complex_algebraic_trig: {
    skillId: "math_m_complex_algebraic_trig",
    day1InitialEvidence_ar: "كتابة z على الشكل الأسي وتطبيق دو موافر لحساب z^6 وإثبات أنه حقيقي.",
