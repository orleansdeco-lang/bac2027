...
| `math_m_complex_argument_loci` | $arg((z-a)/(z-b)) = pi/2 [pi]$ -> Circle $[AB]$ excl. $A,B$ | $arg((z-c)/(z-d)) = 0 [pi]$ -> Line $(CD)$ excl. $C,D$ | Orthogonality vs collinearity via argument; strictly enforces excluded boundary points. |
| `math_m_logarithmic_differentiation` | $[(x^2+1)^x]'$ at $x=1$ -> `2(ln 2 + 1)` | $[(x^2+2)^x]'$ at $x=1$ -> `3(ln 3 + 2/3)` | Exponential transform $exp(x ln u(x))$; chain rule and product derivative at integer point. |
| `math_m_function_study` | Slant asymptote $y=2x-1$, relative position | Slant asymptote $y=3x+2$, relative position | $lim (f(x) - y) = 0$ with sign analysis of remainder term $rac{k}{x-x_0}$. |
| `math_m_bounded_functions` | Integral bounds for $f(x) in [1, 3]$ on $[0, 2]$ -> `[2, 6]` | Integral bounds for $g(x) in [2, 5]$ on $[1, 3]$ -> `[4, 10]` | $m(b-a) le int_a^b f le M(b-a)$; strict positivity and rectangle area bounds. |
